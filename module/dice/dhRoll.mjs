        ...
        return (
            this.options.effects?.reduce((acc, effect) => {
                ...
                if (
                    ...
                    ] : ([this.options.data.parent?.system.attack ?? {}]);
                    const action = actions.find(x => x.id === this.options.source.action);
                    const message = this.options.source.message ? game.messages.get(this.options.source.message) : null;

                    const isConditionalSuppress = action &&
                        (effect.system.conditionals ?? []).some(x => x.constructor.metadata.phase === 'roll' && x.constructor.metadata.failureMode === 'suppress' && !x.test(action.getRollData({ message })));
                    ...
                    acc[effect.id] = {
                        ...
                        origEffect: effect,
                        selected: !isConditionalSuppress && !effect.disabled
                    };
                }
